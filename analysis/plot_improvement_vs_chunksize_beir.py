#!/usr/bin/env python3
"""
Plot showing % improvement (Con-C over Pre-C) vs average chunk size.
...
def collect_scores(
    ...
    """
    Collect scores for each (model, chunker, dataset) combination.

    Returns:
        Dictionary mapping (model, chunker, dataset) -> score
    """
    results = {}
    ...
    for model in models:
        for chunker in chunkers:
            for dataset in datasets:
                if dataset == 'GutenQA':
                    continue  # Skip single-document task
...
def calculate_avg_improvement(
    prec_scores: Dict[Tuple[str, str, str], float],
    conc_scores: Dict[Tuple[str, str, str], float],
    models: List[str],
    chunkers: List[str],
    datasets: List[str]
) -> Dict[Tuple[str, str], float]:
    """
    Calculate average % improvement (Con-C over Pre-C) for each (model, chunker).

    Method: Calculate % improvement for each dataset individually, then average.
    This matches the table calculation method.

    Returns:
        Dictionary mapping (model, chunker) -> average % improvement
    """
    improvements = {}
    ...
    for model in models:
        for chunker in chunkers:
            ...
            if pct_changes:
                improvements[(model, chunker)] = sum(pct_changes) / len(pct_changes)
...
def main():
    ...
    # These are approximate values based on typical chunker behavior
    ...
    print("Collecting Pre-C (RegularEncoder) results...")
    prec_scores = collect_scores(BASE_PATH, DATASETS, MODELS, CHUNKERS, 'RegularEncoder')
    print(f"Found {len(prec_scores)} Pre-C entries")

    print("Collecting Con-C (LateEncoder) results...")
    conc_scores = collect_scores(BASE_PATH, DATASETS, MODELS, CHUNKERS, 'LateEncoder')
    ...
    # Calculate improvements (same method as table: average of per-dataset % improvements)
    print("Calculating improvements (averaging per-dataset % changes)...")
    improvements = calculate_avg_improvement(prec_scores, conc_scores, MODELS, CHUNKERS, DATASETS)
    ...
    output_path = 'figures/rq4_chunksize_vs_improvement_beir.pdf'
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"\nPlot saved to: {output_path}")

    # Also save as PNG for quick viewing
    plt.savefig('figures/rq4_chunksize_vs_improvement_beir.png', dpi=150, bbox_inches='tight')
    ...
    print("\n" + "="*60)
    print("Summary: % Improvement by Model and Chunker")
    print("="*60)
    for model in MODELS:
