#!/usr/bin/env python3
"""
Plot showing absolute Pre-C (RegularEncoder) performance vs average chunk size for BEIR.
...
def collect_beir_average(
    ...
    """Collect BEIR average (excluding GutenQA) for each (model, chunker)."""
    results = {}
    for model in models:
...
def main():
    ...
    print("Collecting Pre-C (RegularEncoder) BEIR results...")
    scores = collect_beir_average(BASE_PATH, DATASETS, MODELS, CHUNKERS, 'RegularEncoder')
    ...
    for idx, model in enumerate(MODELS):
        ...
        if idx == 0:
            ax.set_ylabel('nDCG@10 (Pre-C)', fontsize=18)
    ...
    output_path = 'figures/rq4_chunksize_vs_absolute_prec_beir.pdf'
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"\nPlot saved to: {output_path}")
    plt.savefig('figures/rq4_chunksize_vs_absolute_prec_beir.png', dpi=150, bbox_inches='tight')
