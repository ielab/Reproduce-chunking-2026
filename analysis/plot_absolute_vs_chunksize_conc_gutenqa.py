#!/usr/bin/env python3
"""
Plot showing absolute Con-C (LateEncoder) performance vs average chunk size for GutenQA.
...
def main():
    ...
    scores = collect_gutenqa_scores(BASE_PATH, MODELS, CHUNKERS, 'LateEncoder')
    ...
    for idx, model in enumerate(MODELS):
        ...
        if idx == 0:
            ax.set_ylabel('DCG@10 (Con-C)', fontsize=18)
    ...
    output_path = 'figures/rq4_chunksize_vs_absolute_conc_gutenqa.pdf'
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"\nPlot saved to: {output_path}")
    plt.savefig('figures/rq4_chunksize_vs_absolute_conc_gutenqa.png', dpi=150, bbox_inches='tight')
